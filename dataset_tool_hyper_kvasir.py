...
def open_images_masks_dataset(source_dir, *, max_images: Optional[int], dataset_name=None, target_resolution=(256)):
    ...
    if dataset_name == 'hyper_kvasir':
        meta_fname = os.path.join(source_dir, 'image-labels.csv')
        image_extension = '.jpg'
        mask_extension = '.jpg'
    ...
    # all_classes = df.iloc[:, 2]

    import glob
    
    images = glob.glob(f'{source_dir}/images/*{image_extension}')
    length = len(images)
    max_idx = maybe_min(length, max_images)
    labels = []

    # for file in images:
    #     file = os.path.basename(file).split('.jpg')[0]
    #     x = df.loc[df['Video file'] == file]['Finding'].values[0]
    #     print(file, x)
    #     labels.append(x)
    ...
    def iterate_images():
        for idx in range(len(images)):
            ...
            mask_path = images[idx].replace('images', 'masks')
            ...
            yield dict(img=img, mask=mask, label=0)
